...
// drawShipmentMenu draws the shipment action menu
func (s *GameplayScene) drawShipmentMenu(screen *ebiten.Image) {
	// Semi-transparent overlay
	vector.DrawFilledRect(screen, 0, 0, float32(ScreenWidth), float32(ScreenHeight),
		color.RGBA{0, 0, 0, 180}, false)

	// Determine panel size based on mode
	panelH := 300
	if s.shipmentMode != "" {
		panelH = 400
	}
	...
	if s.shipmentMode == "" {
		...
		s.moveHorseBtn.Y = btnY + 50
		s.moveHorseBtn.Disabled = !hasHorse
		s.moveHorseBtn.Draw(screen)

		// Boat button
		s.moveBoatBtn.X = btnX
		s.moveBoatBtn.Y = btnY + 100
		s.moveBoatBtn.Disabled = !hasBoat
		s.moveBoatBtn.Draw(screen)

		// Cancel/skip button
		s.cancelShipmentBtn.X = btnX
		s.cancelShipmentBtn.Y = btnY + 170
		s.cancelShipmentBtn.Text = "Skip (No Move)"
		s.cancelShipmentBtn.Draw(screen)

		// Instructions
		DrawTextCentered(screen, "Choose what to move this turn", panelX+panelW/2, panelY+panelH-30, ColorTextMuted)

	} else {
		// Show source/destination selection UI
		infoY := btnY

		switch s.shipmentMode {
		case "stockpile":
			// Get stockpile location
			stockpileLoc := "unknown"
			if myPlayer, ok := s.players[s.game.config.PlayerID]; ok {
				player := myPlayer.(map[string]interface{})
			...
			DrawText(screen, fmt.Sprintf("From: %s", stockpileLoc), panelX+20, infoY, ColorText)
			infoY += 25

			destName := "(Click a territory)"
			if s.selectedTerritory != "" {
				if terr, ok := s.territories[s.selectedTerritory].(map[string]interface{}); ok {
					destName = terr["name"].(string)
				}
			}
			DrawText(screen, fmt.Sprintf("To: %s", destName), panelX+20, infoY, ColorText)
			infoY += 40

			DrawText(screen, "Click any owned territory to move", panelX+20, infoY, ColorTextMuted)

		case "horse":
			fromName := "(Click a territory with horse)"
			...
			DrawText(screen, fmt.Sprintf("From: %s", fromName), panelX+20, infoY, ColorText)
			infoY += 25
			...
			// Carry weapon checkbox
			if s.shipmentFromTerritory != "" {
				if terr, ok := s.territories[s.shipmentFromTerritory].(map[string]interface{}); ok {
					hasWeapon, _ := terr["hasWeapon"].(bool)
					if hasWeapon {
						s.drawCheckbox(screen, panelX+20, infoY, "Carry Weapon", &s.shipmentCarryWeapon)
						infoY += 30
					}
				}
			}

			DrawText(screen, "Horse can move up to 2 territories", panelX+20, infoY, ColorTextMuted)

		case "boat":
			fromName := "(Click a territory with boat)"
			if s.shipmentFromTerritory != "" {
				if terr, ok := s.territories[s.shipmentFromTerritory].(map[string]interface{}); ok {
					fromName = terr["name"].(string)
				}
			...
			destName := "(Click coastal destination)"
			if s.selectedTerritory != "" {
				if terr, ok := s.territories[s.selectedTerritory].(map[string]interface{}); ok {
					destName = terr["name"].(string)
				}
			...
			if s.shipmentFromTerritory != "" {
				if terr, ok := s.territories[s.shipmentFromTerritory].(map[string]interface{}); ok {
					...
					if hasHorse {
						s.drawCheckbox(screen, panelX+20, infoY, "Load Horse", &s.shipmentCarryHorse)
						infoY += 25
					}

					if hasWeapon {
						s.drawCheckbox(screen, panelX+20, infoY, "Load Weapon", &s.shipmentCarryWeapon)
						infoY += 25
					}
				}
			}

			DrawText(screen, "Boat stays in same water body", panelX+20, infoY, ColorTextMuted)
			infoY += 20
			DrawText(screen, "Cargo lost if destination has unit", panelX+20, infoY, ColorWarning)
		}

		// Confirm button (only enabled when destination selected)
		s.shipmentConfirmBtn.X = btnX
		s.shipmentConfirmBtn.Y = panelY + panelH - 110
		s.shipmentConfirmBtn.Disabled = s.selectedTerritory == ""
		if s.shipmentMode != "stockpile" && s.shipmentFromTerritory == "" {
			s.shipmentConfirmBtn.Disabled = true
		}
		s.shipmentConfirmBtn.Draw(screen)

		// Cancel button
		s.cancelShipmentBtn.X = btnX
		s.cancelShipmentBtn.Y = panelY + panelH - 60
